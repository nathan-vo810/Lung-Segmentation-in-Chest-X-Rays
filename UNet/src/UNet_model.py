...
from src.process_output import post_processing

DIR = os.path.dirname(__file__)
WEIGHTS_PATH = os.path.join(DIR, "../weights/unet-val-9831.hdf5")
...
class UNet(object):
    ...
    def predict(self, x_test, x_info):
        labels = self.model.predict(x_test, batch_size=1, verbose=1)
        for i in range(labels.shape[0]):
            label = labels[i]
            label = array_to_img(label)

            x_name = str(int(x_info[i][0])) + '.jpg'
            misc.imsave(RESULT_PATH + x_name, label)

            x_shape = x_info[i][1:3]
            data_dir = os.path.join(DIR, '../dataset/training_data')
            result = post_processing(data_dir, x_name, x_shape)

            misc.imsave(RESULT_PATH + x_name, result)
